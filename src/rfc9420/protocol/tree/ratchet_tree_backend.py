...
if TYPE_CHECKING:
    ...
    from ...messages.data_structures import UpdatePath
...
class RatchetTreeBackend(Protocol):
    ...
    def apply_joiner_path_secret(self, joiner_leaf_index: int, path_secret_at_lca: bytes, committer_leaf_index: int = -1) -> None: ...
...
_BACKEND_FACTORIES: dict[
    str, Callable[["CryptoProvider", "RandProviderProtocol | None"], RatchetTreeBackend]
] = {}


def register_tree_backend(
    backend_id: str,
    factory: Callable[["CryptoProvider", "RandProviderProtocol | None"], RatchetTreeBackend],
) -> None:
...
def _ensure_builtin_backends() -> None:
    if _BACKEND_FACTORIES:
        return
    # Lazy imports from same package.
    ...
    register_tree_backend(BACKEND_ARRAY, lambda crypto, rand: RatchetTree(crypto, rand))
    register_tree_backend(
        BACKEND_PERFECT, lambda crypto, rand: PerfectRatchetTree(crypto, rand)
    )
    register_tree_backend(
        BACKEND_LINKED, lambda crypto, rand: LinkedRatchetTree(crypto, rand)
    )
...
def create_tree_backend(
    ...
    rand_provider: "RandProviderProtocol | None" = None,
) -> RatchetTreeBackend:
    ...
    return factory(crypto_provider, rand_provider)
